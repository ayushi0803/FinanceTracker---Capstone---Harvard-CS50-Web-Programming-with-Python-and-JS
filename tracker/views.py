    ...
    if request.method == "POST":
        ...
        if not totalAmount:
            ...
            if float(oldTotalAmount.totalAmount)+newTotalAmount >= 0:
                ...
                totalAmount.save()
    return JsonResponse({"message": "Amount successfully added."})
...
def addExpense(request):
    if request.method == "POST":
        ...
        if float(totalAmount.totalAmount) - newExpense.amount < 0:
            ...
            totalAmount.save()
            newExpense.save()
    return JsonResponse({"message": "Expense successfully recorded."})
...
def getTotalAmount(request):
    try:
        ...
        totalAmount = 0.00
        return JsonResponse({"totalAmount": totalAmount, "message": "Total amount record not found."})
